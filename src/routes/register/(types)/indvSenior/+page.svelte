<script lang="ts">
	...
	let instruments =
		data.props?.details?.instruments ||
...
<div class="flex flex-col items-center justify-center">
	<div class="font-heading mb-10 text-center text-2xl font-bold md:text-4xl lg:text-3xl">
		สมัครเข้าร่วมการแข่งขัน ST Star Challenge 16 ประเภทร้องเดี่ยว ม.ปลาย
	</div>
	<form
		action="?/confirmDetails"
		method="POST"
		on:change={saveProgress}
		class="space-y-4 p-4 text-xl"
	>
		<div>
			<Label for="stageName">รหัสนักเรียน ชื่อ นามสกุล ชั้น/ห้อง เลขที่</Label>
			<Input name="stageName" bind:value={stageName} required></Input>
			<p class="text-sm text-muted-foreground">ตัวอย่าง 40000 นายเอสที สตาร์ ม.6/9 เลขที่ 24</p>
		</div>
		...
		{#if placeholderImageUrl}
			<div>
				<Label for="image">รูปผู้เข้าแข่งขัน (ใบหน้าที่ชัดเจน)</Label>
				<FilePond
					required
		...
		<div class="flex flex-col gap-4">
			...
			{#each [{ name: 'ไมโครโฟนนักร้อง 1', value: instrumentsArray[0] }, { name: 'ไมโครโฟนนักร้อง 2', value: instrumentsArray[1] }, { name: 'กีต้าร์โปร่งไฟฟ้า', value: instrumentsArray[2] }, { name: 'กีต้าร์โปร่งไฟฟ้า 2', value: instrumentsArray[3] }, { name: 'กีต้าร์ไฟฟ้า', value: instrumentsArray[4] }, { name: 'กีต้าร์ไฟฟ้า 2', value: instrumentsArray[5] }, { name: 'เบสไฟฟ้า', value: instrumentsArray[6] }, { name: 'กลองชุดไฟฟ้า', value: instrumentsArray[7] }, { name: 'คาฮอง', value: instrumentsArray[8] }, { name: 'คีย์บอร์ด', value: instrumentsArray[9] }] as instrument, i}
				{#if i === 2 || i === 4 || i === 9}
					<div class="flex items-center space-x-2" id="instrumentsCheckbox">
						<Checkbox
							id={instrument.name}
							name={instrument.name}
							bind:group={instruments}
							onCheckedChange={(e) => modifyInstrumentsArray(i)}
			...
			<AlertDialog.Content>
				<AlertDialog.Header>
					...
					<AlertDialog.Description>
						คุณแน่ใจหรือไม่ว่าต้องการยืนยันการสมัครเข้าร่วมการแข่งขัน ST Star Challenge 16
						ประเภทวงดนตรี เมื่อยืนยันแล้วจะไม่สามารถแก้ไขข้อมูลได้อีก
					</AlertDialog.Description>
				</AlertDialog.Header>
				<AlertDialog.Footer>
